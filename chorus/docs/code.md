...
playback.js — mix graph (createNodes/startGraph/stopGraph), loop=true sources, per-track GainNodes, masterGain → DynamicsCompressor → destination, cancel token
...
playback.js owns:  _graph {nodes[], masterGain, limiter}, _playing, _preloaded, _cancelToken
...
stopPlayback() increments _cancelToken to invalidate in-flight buffer loads; calls stopGraph to disconnect all nodes.
startPlayback(when?) loads buffers, builds graph, starts looping (loop=true); never auto-stops.
...
Master chain: masterGain (default 1.0) → DynamicsCompressor (threshold -6dB, knee 12dB, ratio 10, attack 3ms, release 250ms) → destination.
