...
export default function DragAndDropListManager({ listProps, listStyle, items, onSelect, onDeselect, onDrop }) {
    ...
    const listElement = div({
        ...
        // oncontextmenu: clickAndHold.release, // prevent triggering click and hold on right click
        style: stl({
            padding: 0,
            margin: 0,
            listStyle: 'none',
    ...
    return {
        ...
        insert: (i, el) => i < listElement.children.length
            ? getItemAt(i).insertAdjacentElement('beforebegin', el)
            : appendItem(el),
    ...
    function getEventItem(event) {
        return getItemElement(event.target)
    }

    function getItemElement(element) {
        let el = element
        while (el && el.parentElement !== listElement) {
            el = el.parentElement
        }
        return el
    }
