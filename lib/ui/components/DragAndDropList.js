...
export function DragAndDropList({ items: rawItems, onSelect, onDeselect, onDrop }) {
    const itemSelection = new ItemSelectionManager({
        onSelect: (el) => onSelect(getItemIndex(el)), // get index here otherwise placeholder can mess with has() logic
        onDeselect: (el) => onDeselect(getItemIndex(el)),
    })
    ...
    const listElement = ul({
        ...
        rawItems.map(wrapInItemElement)
    )

    return {
        element: listElement,
        addItem: (el) => listElement.appendChild(wrapInItemElement(el)),
        getElement: (index) => listElement.children[index],
        removeItem: (index) => listElement.children[index].remove(),
    }

    function wrapInItemElement(element) {
        return li({ onpointerenter: (e) => dragAndDrop.hover(getItemElement(e)) }, element)
    }
}
