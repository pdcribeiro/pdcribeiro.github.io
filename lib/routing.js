...
import van, { deriveAsync } from '/lib/ui/web/van-wrapper.js'
import { transformValues } from '/lib/object.js'
...
export default {
  ...
  getPathHelpers(paths) {
    return transformValues({ root: '/', ...paths }, (path) => {
...
export function FileSystemRouter(props) {
  ...
  const page = deriveAsync(() => fetchPageFromFileSystem(url.val), [url])

  const { root, render } = new RenderingManager()

  van.derive(() => {
    if (page.val !== page.oldVal) {
      render(page.val({ ...props, params: getUrlParams() }))
    }
  })
...
function RenderingManager() {
  ...
  return {
    root: elements[0],
    render(page) {
      const newElements = page instanceof DocumentFragment
