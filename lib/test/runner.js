...
export async function test(tests, options = {}) {
    ...
    if (Array.isArray(tests))
        return runSuites(tests, options)
    ...
    if (runningOn.browser() && !options.suite)
        removeTestElements()
...
async function runSuites(suites, options) {
    ...
    for (let tests of suites)
        results.push(await test(tests, { ...options, suite: true }))

    if (results.every(Boolean))
        logColored('\nall suites passed', COLORS.green)
    else
        logColored('\nsome suites failed', COLORS.red)
