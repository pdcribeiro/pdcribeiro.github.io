import { load, reload, remove, setHtml } from './iframe.js'
import { assert } from './runner.js'

const GLOBAL_SELECTOR = '*'
const IGNORED_TAGS = ['HTML', 'BODY', 'STYLE', 'SCRIPT', 'NOSCRIPT', 'TEMPLATE'] // ignore html and body because el.textContent captures script content even if script elements are ignored
const EVENT_OPTIONS = { bubbles: true, cancelable: true }

let iframe

/**
 * @typedef {(...any) => typeof helpers} UiTestHelperFn
 * @type {Object.<string, UiTestHelperFn>}
*/
const helpers = {
    async visit(parent_, url = location.href) {
        iframe = await load(url)
        return helpers.root()
    },
    async reload(parent_) {
        await reload()
        return helpers.root()
    },
    async render(parent_, renderFn) {
        iframe = await setHtml('')
        await renderFn(iframe.contentDocument)
        return helpers.root()
    },
    async renderHtml(parent_, html) {
        iframe = await setHtml(html)
        return helpers.root()
    },
    root(parent_) {
...
function wait(duration = 36e5) {
    return new Promise(resolve => setTimeout(resolve, duration))
}

let wrapLog = (fn, message) => (...args) => {
    let result = fn(...args)
    console.debug(message, result, args)
    return result
}

let findElementByText = wrapLog((parent, text, options = {}) => {
...
function findAllBySelector(parent, selector = GLOBAL_SELECTOR) {
    return Array.from(parent.querySelectorAll(selector))
        .flatMap(el => el.shadowRoot ? findAllBySelector(el.shadowRoot, selector) : el)
...
/** @returns {UiTestHelperFn} */
let createEntryPoint = (helper) =>
    (...args) => createChain(() => (helper(null, ...args)), helpers)

export let visit = createEntryPoint(helpers.visit)
export let render = createEntryPoint(helpers.render)
export let renderHtml = createEntryPoint(helpers.renderHtml)

export { remove as removeTestElements }
